...
- **`vb_*`** — Vertical Bar process mining, all FIXED-SHAPE (Cognito only): `vb_workspaces`, `vb_projects`, `vb_process_overview`, `vb_variants`, `vb_cases`. There is **no `vb_data_query`** — the flexible read-only SQL path over process data is **`briefing_data_query`**, which reaches it through CrossCheck; see the data-path note below. Use `vb_variants` as the single process-variant path; the retired `vb_episode_variants` PoC must not be selected or requested.
