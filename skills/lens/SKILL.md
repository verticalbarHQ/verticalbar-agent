...
- **`lens_publish({ title, account?, html, topic?, note? })`** → `{ id, viewUrl }` — publish the self-contained artifact to the in-product Lens surface. Pass a stable `topic` to stack this as the next version of the same topic (RND-3056). `lens_list` lists published Lenses (metadata only).
