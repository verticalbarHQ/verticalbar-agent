...
6. **Publish — the DEFAULT terminal step (opt-out only).** The end of every analysis is `briefing_publish({title, account, html, workspaceId})` → `{ id, viewUrl, rawUrl, workspace }`; do this unless the user explicitly said not to. **When re-publishing an iteration of an earlier analysis, pass a stable `topic` key** (an optional string) so the platform stacks the new artifact as the next *version* of that same topic instead of a fresh, unrelated briefing (RND-3056); an optional `note` can describe what changed in this version. **Pass the SAME `workspaceId` you chose in scope discovery**; no ambient publish scope exists. The result echoes the resolved `workspace` when available — **confirm it is the workspace the goal targeted** before surfacing the link. `viewUrl` is the human dashboard page `/[org]/[ws]/briefings/<id>` (auto-opened in the browser; `rawUrl` is the auth-gated raw artifact). Surface the `viewUrl` to the user as the deliverable. The artifact carries its own theme (the sandbox can't read the parent's).
...
- **`login({ email?, password? })`** — Cognito login. **No args → opens a browser to sign in (incl. Google)**; or pass **email+password** explicitly for SRP. One login yields a token accepted by both CrossCheck and Vertical Bar. `whoami` shows current auth and workspace routing mode.
...
requires an identified Cognito user. The installed plugin always targets production. Demo order:
`login` → resolve the snapshot Git source → create package →
